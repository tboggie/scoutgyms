...
const deptData = [
  { name: 'Engineering', rate: 82 },
  { name: 'Design', rate: 91 },
  { name: 'Sales', rate: 68 },
  { name: 'Marketing', rate: 74 },
  { name: 'HR', rate: 88 },
  { name: 'Finance', rate: 61 },
  { name: 'Operations', rate: 72 },
];
  ...
  return (
    <DashboardShell variant="business">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-card rounded-xl border border-border p-5"><p className="text-xs text-text3 font-medium mb-1">Monthly cost</p><p className="font-[var(--font-display)] text-3xl font-bold text-purple">${s.monthlySpend.toLocaleString()}</p><p className="text-xs text-text3 mt-1">${s.costPerMember}/member</p></div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        <h2 className="font-semibold mb-4">Utilisation by department</h2>
        {deptData.map(d=>(
          <div key={d.name} className="flex items-center justify-between py-3 border-b border-border last:border-none">
            <span className="text-sm text-text2">{d.name}</span>
            <div className="flex items-center gap-3">
              <div className="w-32 h-2 rounded-full bg-border overflow-hidden"><div className="h-full rounded-full bg-teal" style={{width:`${d.rate}%`}}/></div>
              <span className="text-sm font-semibold w-10 text-right">{d.rate}%</span>
            </div>
          </div>
        ))}
      </div>
